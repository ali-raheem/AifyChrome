...
async function generate(draftContainer, messages, draftTitle) {
    ...
    try {
        const { apiKey, model, maxTokens } = await chrome.storage.local.get(
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const { messages, draftTitle } = await chrome.storage.local.get(["messages", "draftTitle"]);
    regenButton.addEventListener("click", () => generate(draftContainer, messages, draftTitle));
    chatButton.addEventListener("click", async () => {
        await chrome.storage.local.set({messages});
        await chrome.windows.create({ url: "/html/chat.html", type: "popup",
