...
async function callBackendAPI(original, action) {
    const { model, apiKey, maxTokens } = await chrome.storage.local.get(["model", "apiKey", "maxTokens"]);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const { original, action, draftTitle } = await chrome.storage.local.get(["original", "action", "draftTitle"]);
